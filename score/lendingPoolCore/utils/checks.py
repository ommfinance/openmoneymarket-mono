...
def only_delegation(func):
    ...
    def __wrapper(self: object, *args, **kwargs):
        if self.msg.sender != self._delegation.get():
            revert(f"{TAG}: "f"SenderNotAuthorized: (sender){self.msg.sender} (delegation){self._delegation.get()}")
...
def only_liquidation_manager(func):
    ...
    def __wrapper(self: object, *args, **kwargs):
        if self.msg.sender != self._liquidation.get():
            revert(f"{TAG}: "f"SenderNotAuthorized: (sender){self.msg.sender} (liquidation){self._liquidation.get()}")
...
def only_governance(func):
    ...
    def __wrapper(self: object, *args, **kwargs):
        if self.msg.sender != self._governance.get():
            revert(f"{TAG}: "f"SenderNotAuthorized: (sender){self.msg.sender} (governance){self._governance.get()}")
