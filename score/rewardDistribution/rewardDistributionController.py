...
class RewardDistributionController(RewardDistributionManager):
    ...
    def distribute(self) -> None:
        ...
        if not self._precompute['ommICX']:
            ...
            self._totalAmount['ommICX'] = data_source.getTotalValue("OMM3/sICX", day)
            self._precompute['ommICX'] = True

        elif not self._distComplete['ommICX'] and self._check('ommICX'):
            self.State("distribute ommICX")
            data_source = self.create_interface_score(self._lpTokenAddress.get(), DataSourceInterface)
            data_batch = data_source.getDataBatch("OMM3/sICX", day, BATCH_SIZE, self._offset["OMMSICX"])
            ...
            data_source = self.create_interface_score(self._lpTokenAddress.get(), DataSourceInterface)
            self._totalAmount['dex'] = convertToExa(data_source.getTotalValue("OMM3/IUSDC", day), IUSDC_PRECISION)
            self._totalAmount['dex'] += data_source.getTotalValue("OMM3/USDS", day)
            self._precompute['dex'] = True

        elif not self._distComplete['dex'] and self._check('dex'):
            self.State("distribute dex")
            data_source = self.create_interface_score(self._lpTokenAddress.get(), DataSourceInterface)
            data_batch1 = data_source.getDataBatch("OMM3/IUSDC", day, BATCH_SIZE, self._offset["dex"])
            data_batch2 = data_source.getDataBatch("OMM3/USDS", day, BATCH_SIZE, self._offset["dex"])
