...
class TestIntegrationDepositUSDb(IconIntegrateTestBase):
    ...
    def setUp(self):
        ...
        self.sicxAmount = 1000 * 10 ** 18
        self._transferSicxToUser(self.sicxAmount,self.test_account2.get_address())

        self._transferTestICXToUser(10000 * 10**18,self.test_account2.get_address())

        self.InitialICXDeposit = 1000 * 10**18
        self._depositICX(self.InitialICXDeposit, self.test_account2)



        self.UsdbDeposit = 800 * 10**18
        self._depositUSDb(self.UsdbDeposit, self._test1)

        self.ICXDeposit = 200 * 10**18
        self._depositICX(self.ICXDeposit, self._test1)

        self.ICXborrowAmount=400 * 10**18
        self._borrowSICX(self.ICXborrowAmount,self._test1)


        self.borrowUSDb2=997 * 10**17
        ...
        # self.icxRedeemAmount = 4872 * 10**17
        # self._redeemIXC(self.icxRedeemAmount,self._test1)

        

        self._changeOraclePriceFeed('sICX',17 * 10**17) 
        

       
        self.loanPayoffAmount = 12358 * 10 ** 16
        self._liquidationCall(self.loanPayoffAmount, self._test1, self.test_account2,self.contracts['sample_token'],self.contracts['sicx'])
       

        UsdbReserveData = self.getReserveData(self.contracts['sample_token'])
        # pprint(reserveData)
        for key,value in UsdbReserveData.items():
            if "Address" not in key:
                UsdbReserveData[key] = UsdbReserveData[key] / 10**18
        print("USDb")        
        pprint(UsdbReserveData)


        ICXreserveData = self.getReserveData(self.contracts['sicx'])
        # pprint(reserveData)
        for key,value in ICXreserveData.items():
            if "Address" not in key:
                ICXreserveData[key] = ICXreserveData[key] / 10**18
        print("ICX")
        pprint(ICXreserveData)
    ...
    def _changeOraclePriceFeed(self,_reserve:str,rate:int):
        settings = [{'contract': 'priceOracle', 'method': 'set_reference_data',
                     'params': {'_base': _reserve, '_quote': 'USD', '_rate': rate}}]
    ...
    def _transferSicxToUser(self, _testFund: int, _user):
        # Transferring 1 Mill USdb to  test_account3
        params = {"_to": _user,"_value": _testFund}
        call_transaction = CallTransactionBuilder() \
            .from_(self._test1.get_address()) \
    ...
    def _redeemIXC(self, _redeemAmount : int,_user):
    ...
    def _redeemUSDb(self, _redeemAmount : int,_user):
        ...
        call_transaction = CallTransactionBuilder() \
            ...
            .params(params) \
            .build()
        signed_transaction = SignedTransaction(call_transaction, self._user)
        tx_result = self.process_transaction(signed_transaction)
        self.assertTrue('status' in tx_result)
        self.assertEqual(1, tx_result['status']) 
    ...
    def _liquidationCall(self, _loanPayoffAmount : int, _borrowerUser : KeyWallet, _liquidatorUser : KeyWallet,_collateral:Address,_reserve:Address) :
