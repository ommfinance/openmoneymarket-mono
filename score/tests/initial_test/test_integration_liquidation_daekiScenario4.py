...
class TestIntegrationDepositUSDb(IconIntegrateTestBase):
    ...
    def _liquidationCall(self, _loanPayoffAmount : int, _borrowerUser : KeyWallet, _liquidatorUser : KeyWallet) :
        ...
        print('sicx reserve available liquidity before', self.getReserveData(self.contracts['sicx'])['availableLiquidity'])
        liquidityReservebefore = self.getReserveData(self.contracts['sicx'])['availableLiquidity']
        ...
        self.assertTrue('status' in tx_result)
        self.assertEqual(1, tx_result['status'])
        print('sicx reserve available liquidity after liquidation', self.getReserveData(self.contracts['sicx']))
        liquidityReserveafter = self.getReserveData(self.contracts['sicx'])['availableLiquidity']
