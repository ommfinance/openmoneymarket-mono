...
class TestIntegrationDepositUSDb(IconIntegrateTestBase):
    ...
    def setUp(self):
        ...
        #Trasnfer 2000 sICX to test_account3
        self.userTestSICXAmount = 2000 * 10 ** 18
        self._transferTestSICXToUser(self.userTestSICXAmount, self.test_account3.get_address())
        
        #-------------------#
        #test_account3 deposits 1000 ICX 
        self.depositICXAmount1 = 1000 * 10 ** 18 
        self._depositICX(self.depositICXAmount1, self.test_account3)

        #test_account2 deposits 1000 USDb 
        self.depositUSDbAmount1 = 1000 * 10 ** 18 
        self._depositUSDb(self.depositUSDbAmount1, self.test_account2)

        #test_account2 borrows 200 USDb        
        self.borrowUSDbAmount1 = 200 * 10 ** 18 
        self._borrowUSDb(self.borrowUSDbAmount1, self.test_account2)

        #test_account2 borrows 200 ICX        
        self.borrowSICXAmount1 = 200 * 10 ** 18 
        self._borrowSICX(self.borrowSICXAmount1, self.test_account2)

        #self.liquidationCall() #user cannot get liquidated 
        #reduce icxUSD rate  
        self.sICXRate = 3 * 10 ** 18
        self._changeOraclePriceFeed(self.sICXRate) 
        
        #test_account3 pays off 1k loan from user2  
        self.loanPayoffAmount = 200 * 10 ** 18 
    ...
    def _liquidationCall(self, _loanPayoffAmount : int, _borrowerUser : KeyWallet, _liquidatorUser : KeyWallet) :
        ...
        print('sicx reserve available liquidity before', self.getReserveData(self.contracts['sample_token']))
        liquidityReserveBefore = self.getReserveData(self.contracts['sample_token'])['availableLiquidity']
        print('_liquidatorUserInitial sicx amount', self.getUserSICXBalance(_liquidatorUser.get_address())/10**18)
        print('_liquidatorUserInitial usdb amount', self.getUserUSDbBalance(_liquidatorUser.get_address())/10**18)
        ...
        self.assertTrue('status' in tx_result)
        self.assertEqual(1, tx_result['status'])
        print('sicx reserve available liquidity after liquidation', self.getReserveData(self.contracts['sample_token']))
        liquidityReserveAfter = self.getReserveData(self.contracts['sample_token'])['availableLiquidity']
        print('change in liquidity is', (liquidityReserveAfter - liquidityReserveBefore)/10**18)
        ...
        print('_liquidatorUserInitial usdb amount', self.getUserUSDbBalance(_liquidatorUser.get_address())/10**18)
        print('oUSDb balance of borrower before', self.getUserOUSDbBalance(_borrowerUser.get_address())/10**18)
        print('sicx balance of borrower before', self.getUserSICXBalance(_borrowerUser.get_address())/10**18)
