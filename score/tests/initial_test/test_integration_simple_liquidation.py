...
class TestIntegrationDepositUSDb(IconIntegrateTestBase):
    ...
    def setUp(self):
        ...
        self.borrowUSDb = 330 * 10 ** 18
        self._borrowUSDb(self.borrowUSDb,self._test1)

        # self.borrowIUSDC = 40 * 10 ** 6
        # self._borrowIUSDC(self.borrowIUSDC,self._test1)

        self.withDrawICX = 4872 * 10 ** 17
        self._redeemIXC(self.withDrawICX,self._test1)
        ...
        self._changeOraclePriceFeed('sICX',8 * 10**17) 
        userAccountData=self.getUserAccountData(self._test1.get_address())
        for key,value in userAccountData.items():
            if "USD" in key:
                userAccountData[key]=value / 10 ** 18
        pprint(userAccountData)
        liquidationList = self._liquidationList()
        pprint(liquidationList)
        self.loanPayoffAmount = 1946 * 10**17
        self._liquidationCall(self.loanPayoffAmount, self._test1, self.test_account2,self.contracts['sicx'],self.contracts['sample_token'])
        userAccountData=self.getUserAccountData(self._test1.get_address())
        for key,value in userAccountData.items():
            if "USD" in key:
                userAccountData[key]=value / 10 ** 18
        pprint(userAccountData)
        userReserveData = self.getUserReserveData(self.contracts['sicx'],self._test1.get_address())
        pprint(userReserveData)
        userReserveData = self.getUserReserveData(self.contracts['sample_token'],self._test1.get_address())
        pprint(userReserveData)
