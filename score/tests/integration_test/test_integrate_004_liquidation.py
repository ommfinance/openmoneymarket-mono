...
class LiquidationTest(OMMTestCases):
	...
	def _execute(self, task):
		...
		for case in task["transaction"]:
			...
			if _step == Steps.DEPOSIT_ICX:
			...
			elif _step == Steps.BORROW_ICX or _step == Steps.BORROW_USDB:
				...
				if self.values["before"]["borrower"]["account_data"].get("healthFactorBelowThreshold") == "0x1":
					print("IN LIQUIDATION STATE")
