...
class TokenTransfer(OmmUtils):
    ...
    def _balances(self, addr):
        a = self.call_tx(to=self.contracts['usds'],method="balanceOf",params={'_owner': addr})
    ...
    def _execute(self, task):
        ...
        for case in task.get("transaction"):
            ...
            if _step == Steps.DEPOSIT_USDS:
                ...
                tx_res = self._transferOUSDS(self._user, self._to.get_address(), self._amount)
                self._check_tx_result(tx_res, case)
                ...
                tx_res = self._transferDICX(self._user, self._to.get_address(), self._amount)
                self._check_tx_result(tx_res, case)
