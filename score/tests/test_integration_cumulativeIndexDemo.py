...
class TestIntegrationDepositUSDb(IconIntegrateTestBase):
    ...
    def setUp(self):
        ...
        t1B1=self.getReserveData(self.contracts['sample_token'])     
        print("LCI before t1",t1B1['liquidityCumulativeIndex']/10**18) 
        self.depositUSDbAmountUser2 = 40000 * 10 ** 18
        self._depositUSDb(self.depositUSDbAmountUser2, self.test_account2)
        t1A1=self.getReserveData(self.contracts['sample_token'])
        print("LCI after t1",t1A1['liquidityCumulativeIndex']/10**18)

        self.borrowUSDb2=10000 * 10 ** 18
        self._borrowUSDb(self.borrowUSDb2,self.test_account2)

        
        t2A1=self.getReserveData(self.contracts['sample_token'])
        print("LCI after t2",t2A1['liquidityCumulativeIndex']/10**18)
        time.sleep(5)
        #test_account3 deposits 500 ICX into sicx reserve
        self.depositAmountUser3 = 500 * 10 ** 18 
        self._depositUSDb(self.depositAmountUser3, self.test_account3)

        t3A2=self.getReserveData(self.contracts['sample_token'])
        print("LCI after t2",t3A2['liquidityCumulativeIndex']/10**18)
