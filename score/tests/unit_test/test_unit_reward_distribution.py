...
REWARD_CONFIG_1 = {
    "_asset": ASSET_ADDRESS_1,
    "_address": ASSET_ADDRESS_1,
    "_percentage": 11 * EXA // 10,
    "totalSupply": 83 * EXA
}

ASSET_CONFIG_2 = {
    "_asset": ASSET_ADDRESS_2,
    "_percentage": 12 * EXA // 10,
...
class TestRewardDistributionController(ScoreTestCase):
    ...
    def _setup_asset_emission(self, account: Address, config):
        ...
        self.patch_internal_method(config["_address"], "getTotalStakedBalance", lambda _asset: config["totalSupply"])

        self.score.configureEmissionPerSecond([{
    ...
    def test_configure_asset_emission(self):
        ...
        with mock.patch.object(self.score, "now", return_value=_mock_time_elapsed):
            ...
            self.assert_internal_call(REWARD_CONFIG_1['_address'], "getTotalStakedBalance", REWARD_CONFIG_1['_asset'])
    ...
    def test_set_distribution_percentage(self):
        ...
        actual_result = self.score.getDailyDistributionPercentage("worker")
    ...
    def test_handle_action_case(self):
        ...
        _asset_config = {
            "_asset": ASSET_ADDRESS_1,
            "_percentage": 55 * EXA // 100,
            "_address": create_address(AddressPrefix.CONTRACT),
            "totalSupply": 0
        }
        ...
        self.assertEqual(0, self.score._userIndex[_user1][ASSET_ADDRESS_1])
        self.assertAlmostEqual(763.888888888889, self.score._userIndex[_user2][ASSET_ADDRESS_1]/EXA,10)
        self.assertAlmostEqual(763.888888888889, self.score._assetIndex[ASSET_ADDRESS_1]/EXA,10)
        ...
        self.assertEqual(1600 * EXA, self.score._userIndex[_user1][ASSET_ADDRESS_1])
        self.assertEqual(1200 * EXA, self.score._userIndex[_user2][ASSET_ADDRESS_1])
        self.assertEqual(1600 * EXA, self.score._assetIndex[ASSET_ADDRESS_1])
        ...
        self.assertEqual(1600 * EXA, self.score._userIndex[_user1][ASSET_ADDRESS_1])
        self.assertEqual(1600 * EXA, self.score._userIndex[_user2][ASSET_ADDRESS_1])
        self.assertEqual(1600 * EXA, self.score._assetIndex[ASSET_ADDRESS_1])

        _last_updated_timestamp = _mock_time_elapsed  ##700

    def _call_handle_action(self, _user, _asset, _time, _old_index):
        _asset_address = _asset["config"]["_asset"]
        _percentage = _asset["config"]["_percentage"]
    ...
    def test_get_rewards_balance_1(self):
        ...
        _expected_result = {'daoFund': 40 * EXA,
                            'depositBorrowRewards': 2000 * EXA,
                            'dex': 20 * EXA,
                            'liquidityRewards': 50 * EXA,
                            'ommICX': 30 * EXA,
                            'ommRewards': 2050 * EXA,
                            'total': 2100 * EXA,
                            'worker': 10 * EXA}
        ...
        _expected_result = {'daoFund': 80 * EXA,
                            'depositBorrowRewards': 3000 * EXA,
                            'dex': 40 * EXA,
                            'liquidityRewards': 100 * EXA,
                            'ommICX': 60 * EXA,
                            'ommRewards': 3100 * EXA,
                            'total': 3200 * EXA,
                            'worker': 20 * EXA}
    ...
    def test_get_rewards_balance_2(self):
        ...
        _expected_result = {'daoFund': 32 * EXA,
                            'depositBorrowRewards': 4000 * EXA,
                            'dex': 15 * EXA,
                            'liquidityRewards': 60 * EXA,
                            'ommICX': 45 * EXA,
                            'ommRewards': 4037 * EXA,
                            'total': 4097 * EXA,
                            'worker': 5 * EXA}
        ...
        _expected_result = {'daoFund': 4 * EXA,
                            'depositBorrowRewards': 9100 * EXA,
                            'dex': 6 * EXA,
                            'liquidityRewards': 14 * EXA,
                            'ommICX': 8 * EXA,
                            'ommRewards': 9114 * EXA,
                            'total': 9128 * EXA,
                            'worker': 10 * EXA}
    ...
    def _call_get_rewards_balance(self, _user, _mock_time_elapsed, _supply):
        with mock.patch.object(self.score, "now", return_value=_mock_time_elapsed):
            ...
            self.assert_internal_call(self.mock_lending_pool_core, "getAssetPrincipalSupply", REWARD_CONFIG_1["asset"],
                                      _user)
    ...
    def test_claim_rewards_case1(self):
        ...
        with mock.patch.object(self.score, "now", return_value=_mock_time_elapsed):
            ...
            self.assert_internal_call(self.mock_lending_pool_core, "getAssetPrincipalSupply", ASSET_ADDRESS_1,
                                      _user1)
